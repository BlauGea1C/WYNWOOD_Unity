...
public class Outline : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // Forzar valores compatibles con cualquier material
        outlineFillMaterial.name = "OutlineFill (Instance)";
        outlineFillMaterial.renderQueue = 5000;
        outlineFillMaterial.SetInt("_ZWrite", 0);
        outlineFillMaterial.SetInt("_ZTest", (int)UnityEngine.Rendering.CompareFunction.Always);

        outlineMaskMaterial.name = "OutlineMask (Instance)";
        outlineMaskMaterial.renderQueue = 4999;
    ...
    void LoadSmoothNormals()
    {
        foreach (var meshFilter in GetComponentsInChildren<MeshFilter>())
        {
            ...
            var smoothNormals = (index >= 0) ? bakeValues[index].data : SmoothNormals(meshFilter.sharedMesh);
            meshFilter.sharedMesh.SetUVs(3, smoothNormals);
            CombineSubmeshes(meshFilter.sharedMesh, meshFilter.GetComponent<Renderer>()?.sharedMaterials);
        }
